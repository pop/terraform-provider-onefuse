...
pipeline {
    ...
    parameters {
        string(name: 'version', defaultValue: "${sh(returnStdout: true, script: "git tag --contains | head -1").trim()}", description: 'Release Folder Name. Version is set in the VERSION file of the repository.')
        ...
        string(name: 'release_date', defaultValue: "${new Date().format( 'YYYY-MM-dd' )}", description: 'Release date of artifact.')
    }
    environment {
      VERSION = sh(
          script: "cat VERSION",
